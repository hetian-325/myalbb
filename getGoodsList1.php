<?php
	...
	//2、数据保存在数据库中
	//1）、建立连接（搭桥）
	$conn = mysql_connect("localhost","xian","qianfeng");
	
	//2）、选择数据库（找目的地）
	if(!mysql_select_db("myshop1",$conn)){
		die("数据库选择失败".mysql_error());
	}
	
	//3）、传输数据（过桥）
	$sqlstr = "select * from goodsInfo1 order by goodsId1";
	...
	while($query_row){
		$str = $str.'{ "goodsId1":"'.$query_row[0].'","goodsName":"'.$query_row[1].'"
